...
- Project-level submission packages now include `submission_manifest.json`, `submission_archive.zip`, `submission_archive_manifest.json`, `reproducibility_checklist.md`, `reproducibility_checklist.json`, `reviewer_response.md`, `project_review_findings.json`, `repair_execution_log.json`, `claim_evidence_index.md`, `retrieval_evidence_ledger.json`, `lineage_archive.json`, `literature_support_index.json`, `paper_compiler_evidence.json`, `publication_evidence_index.json`, `publication_readiness_report.json`, `supplemental_artifacts.json`, `benchmark_card.json`, `benchmark_provenance_manifest.json`, `benchmark_provenance_repair_index.json`, `statistics_report.json`, `experiment_repair_index.json`, `negative_evidence_report.json`, `limitations_appendix.json`, `artifact_integrity_audit.json`, `final_publish_decision.json`, `offline_publication_case.json`, `offline_publication_audit.json`, `code_package.zip`, and `publication_manifest.json`.
...
- Goal 8 evaluation now persists one deterministic trace artifact per evaluation case plus `evaluation_case_audit.json`, `system_metrics.json`, and `scholarflow_system_paper_material.json`. The claim-evidence vertical trace links the SciFact verification/retrieval snapshots, retrieval evidence ledger, project paper sources, revision/re-review artifacts, submission archive manifest, reproducibility checklist, artifact-integrity audit, negative evidence report, and final publish decision into the system evaluation material.
- Goal 8 system-paper material is labeled as evaluation-backed system paper material, not a final submission. Claims about evidence-constrained routing, artifact lineage, failure honesty, revision/package readiness, and final-gate behavior must cite trace artifacts or computed metrics; live external literature/execution, long-running autonomy, multi-project memory, compliance, and venue release remain limitations/future work.
...
- Goal 7 adds a typed submission package and final publish gate. `submission_archive_manifest.json` reconstructs the zip from source artifacts with hashes and validation status; `reproducibility_checklist.json` is generated from runtime/evidence manifests; `artifact_integrity_audit.json` blocks missing, stale, or hash-mismatched artifacts; and `final_publish_decision.json` is the single source for `project_final_publish_ready`.
- The final archive download path remains gated: a review-ready package can have a complete/current archive for audit, but download as a final-publish package stays blocked unless the final decision passes all literature, benchmark independence, execution, statistics, negative-evidence, claim coverage, revision, reproducibility, archive, and integrity checks.
...
- `literature_support_index.json` now records FARS/ARIS related-system coverage from cached/imported literature metadata and known-SOTA fields, and explicitly marks missing coverage as a limitation instead of inventing related-work support. Cached arXiv, Semantic Scholar, and Crossref papers carry source ids, cache keys/timestamps, fingerprints, extraction status, connector availability, and source sufficiency policy so abstract-only or metadata-only evidence cannot silently support novelty/final-publish claims.
...
- Goal 3 adds a typed experiment-job runtime beside, not inside, the queue/worker execution plane. `AutoResearchExecutionPlane` still schedules whole auto-research runs; `experiment_execution.py` materializes domain protocols into typed experiment jobs and produces experiment evidence.
- Typed execution jobs now record `deterministic_replay`, `local_command`, `docker`, `external_import`, or `bridge_import` routes, runtime contracts, expected inputs/outputs, metric schemas, environment requirements, approval state, lineage refs, claim ceilings, and structured blockers. Unsupported domains, missing protocol/benchmark, Docker unavailable, bridge unavailable, and unapproved budget routes stop with auditable blockers instead of fake outputs.
- The typed runtime writes `experiment_execution_plan.json` and `experiment_execution_result.json`. Results include environment manifests, execution profiles, runtime contract results, output validation with hashes/schema/metric/sample/split/baseline/ablation fields, deterministic fingerprints, failure classifications, repair recommendations, negative evidence, package-manifest fragments, and evidence-ledger entries. Project-level `experiment_repair_index.json` now records `typed_experiment_execution_run_ids` and per-run typed execution capsules alongside imported replay and materialized factory surrogates.
...
- Project-level source packages now include a versioned/fingerprinted `manuscript_context.json`, source `claim_evidence_index.json`, `artifact_index.json`, `figures_tables_metadata.json`, `source_fingerprints.json`, and an enriched `manifest.json` with file hashes, reconstructability, external artifact completeness, and missing-reference/artifact readiness blockers. Local compiler absence still leaves a complete auditable source package when these source/evidence files are present.
- Goal 6 now materializes a typed autonomous revision loop for project papers. `paper_original.md` preserves the pre-revision manuscript, `revision_action_plan.json` maps every reviewer finding to a bounded revision action or explicit blocker with evidence requirement, lineage refs, expected outputs, attempt limits, and status, `reviewer_response_dossier.json` records finding-by-finding responses without claiming new evidence unless artifact refs exist, and `revision_round.json` records original/revised fingerprints, action executions, re-review resolution counts, terminal status, readiness impact, and unresolved blockers.
- Claim downgrades and paper-only revisions can be applied deterministically to the manuscript/source package and source claim-evidence index, while experiment, literature, benchmark/provenance, and reproducibility follow-ups remain blocked or pending when no validated evidence artifact already exists. The loop preserves the existing review/repair/rereview path instead of replacing it with a parallel reviewer system.
...
- Project-level re-review reports now carry previous review, revised artifact, and re-review fingerprints so the revision round can prove it consumed revised artifacts rather than replaying an old review object.
...
- Supported-domain benchmark resolvers can now consume brief-provided imported/local benchmark provenance. Passing base publication-grade provenance raises resolver readiness, but does not bypass source independence, statistics, execution validation, negative evidence, or publish-gate blockers.
- The claim-evidence evaluation trace now records submission-package V3, Goal 5, Goal 6, and Goal 7 proof fields, including generated asset roles, missing required roles, source manifest path, source-package reconstructability/readiness, missing source/external artifact lists, manuscript context path/fingerprint/completeness, submission archive manifest/path/current/complete/download readiness, reproducibility checklist completeness/missing/partial counts, artifact-integrity audit completeness/unresolved count, final-publish policy version/failed check ids, publication/readiness/statistics/experiment-repair/negative-evidence/offline-case/offline-audit/review-findings paths, revision action plan path, reviewer response dossier path, revision round path, selected revision action ids, paper-only action ids, blocked evidence-action ids, reviewer response item count, re-review resolved/partial/unresolved/regressed counts, revision terminal status, revision readiness impact, review finding count and action mapping, execution source counts, materialized execution run ids, paper section coverage, claim-support counts, claim ceiling, literature source sufficiency, connector availability, extraction limitations, negative-evidence coverage, Phase 6 covered/missing/required category fields, runtime-failure observation status, final-publish package completeness, engineering/scientific gap counts and classifications, Phase 1 blocked requirement ids, benchmark schema coverage/blockers, benchmark source-observation coverage/blockers, benchmark final-publish-candidate blockers, benchmark source-independence blockers, kill criteria, required follow-ups, and negative-evidence counts.
...
- The old toy/materialized factory path remains a deterministic surrogate with an explicit claim ceiling. It is not relabeled as production experiment execution, and fixture/local smoke evidence remains review-only unless real/imported benchmark provenance, sufficient statistics, negative evidence, and publish-gate requirements are satisfied.
...
- Materialize project-level paper sources with Markdown, LaTeX, bibliography, compile report, build script, versioned manuscript context, source claim-evidence index, artifact index, figures/tables metadata, source fingerprints, original/revised manuscript snapshots, revision action index, typed autonomous revision action plan, reviewer response dossier, revision round report, revision application report, and re-review report.
- Materialize a review-ready submission package with reproducibility checklist, reviewer response, claim-evidence index, lineage archive, supplemental artifacts manifest, benchmark card, code package, publication manifest, and Goal 6 revision artifacts.
...
- Literature coverage now exercises multi-source cached connectors with provenance and extraction limits, but must still be expanded to broader real-paper and full-text coverage before a high-level related-work or novelty claim is made.
