...
def test_autoresearch_text_run_generates_grounded_paper(monkeypatch, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert Path(run["paper_revision_action_index_path"]).is_file()
        assert Path(run["paper_revision_diff_path"]).is_file()
        ...
        assert run["paper_revision_action_index"]["revision_round"] == run["paper_revision_state"]["revision_round"]
        assert run["paper_revision_action_index"]["pending_action_count"] == len(
            run["paper_revision_state"]["next_actions"]
        )
        assert run["paper_revision_action_index"]["completed_action_count"] == 0
        assert run["paper_revision_action_index"]["materialized_action_count"] >= 1
        assert run["paper_revision_diff"]["revision_round"] == run["paper_revision_state"]["revision_round"]
        assert run["paper_revision_diff"]["base_revision_round"] is None
        assert run["paper_revision_diff"]["changed_section_count"] == len(run["paper_plan"]["sections"])
        assert run["paper_revision_diff"]["resolved_action_count"] == 0
        assert run["paper_revision_diff"]["resolved_issue_count"] == 0
        assert any(item["status"] == "initial" for item in run["paper_revision_diff"]["sections"])
        rewrite_packets = {
        ...
        assert "paper_revision_action_index.json" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        assert "paper_revision_diff.json" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        assert "paper_sources/paper_compile_report.json" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        assert "paper_sources/paper_revision_action_index.json" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        assert "paper_sources/paper_revision_diff.json" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        ...
        assert "revision_diff.md" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        assert "revision_actions.md" in run["paper_revision_state"]["checkpoints"][0]["relative_assets"]
        ...
        assert "paper_revision_action_index.json" in paper_sources_files
        assert "paper_revision_diff.json" in paper_sources_files
        assert "revision_actions.md" in paper_sources_files
        assert "revision_diff.md" in paper_sources_files
        ...
        assert (Path(run["paper_sources_dir"]) / "paper_revision_action_index.json").is_file()
        assert (Path(run["paper_sources_dir"]) / "paper_revision_diff.json").is_file()
        assert (Path(run["paper_sources_dir"]) / "revision_actions.md").is_file()
        assert (Path(run["paper_sources_dir"]) / "revision_diff.md").is_file()
        ...
        revision_actions = Path(run["paper_sources_dir"]) / "revision_actions.md"
        assert revision_actions.is_file()
        assert "# Revision Actions" in revision_actions.read_text(encoding="utf-8")
        revision_diff = Path(run["paper_sources_dir"]) / "revision_diff.md"
        assert revision_diff.is_file()
        assert "# Revision Diff" in revision_diff.read_text(encoding="utf-8")
        ...
        assert (Path(run["paper_sources_dir"]) / "checkpoints" / "round_0000" / "revision_actions.md").is_file()
        assert (Path(run["paper_sources_dir"]) / "checkpoints" / "round_0000" / "revision_diff.md").is_file()
        assert (Path(run["paper_sources_dir"]) / "checkpoints" / "round_0000" / "revision_brief.md").is_file()
        assert (Path(run["paper_sources_dir"]) / "checkpoints" / "round_0000" / "paper_revision_action_index.json").is_file()
        assert (Path(run["paper_sources_dir"]) / "checkpoints" / "round_0000" / "paper_revision_diff.json").is_file()
...
def test_autoresearch_registry_exposes_run_lineage_and_candidate_manifests(
    ...
    try:
        ...
        assert registry["files"]["paper_revision_action_index_json"]["exists"] is True
        assert registry["files"]["paper_revision_diff_json"]["exists"] is True
        ...
        assert any(
            edge["relation"] == "has_asset" and edge["target_kind"] == "paper_revision_action_index"
            for edge in registry["lineage"]["edges"]
        )
        assert any(
            edge["relation"] == "has_asset" and edge["target_kind"] == "paper_revision_diff"
            for edge in registry["lineage"]["edges"]
        )
        assert any(
        ...
        assert any(
            edge["relation"] == "materialized_to_run_asset" and edge["target_kind"] == "paper_revision_action_index"
            for edge in candidate_registry["lineage"]["edges"]
        )
        assert any(
            edge["relation"] == "materialized_to_run_asset" and edge["target_kind"] == "paper_revision_diff"
            for edge in candidate_registry["lineage"]["edges"]
        )
        assert any(
...
def test_autoresearch_bundle_index_exposes_selected_and_portfolio_assets(
    ...
    try:
        ...
        assert any(item["role"] == "run_paper_revision_action_index_json" for item in selected_bundle["assets"])
        assert any(item["role"] == "run_paper_revision_diff_json" for item in selected_bundle["assets"])
...
def test_autoresearch_review_loop_refresh_command_updates_rounds_and_resolution_state(
    monkeypatch,
    tmp_path: Path,
) -> None:
    client = _configure_test_client(monkeypatch, tmp_path)
    try:
        project_id = _create_project(
            client,
            "Phase 5 Review Refresh Command Project",
            "Automatic topic classification for compact CS abstracts",
        )
        run_response = client.post(
            f"/api/projects/{project_id}/auto-research/run",
        ...
        first_loop = client.post(
            f"/api/projects/{project_id}/auto-research/{run_id}/review-loop/refresh"
        ).json()
        assert first_loop["current_round"] == 1
        assert first_loop["pending_action_count"] >= 1
        first_fingerprint = first_loop["latest_review_fingerprint"]
...
def test_autoresearch_paper_revision_state_tracks_review_loop_progress(
    ...
    try:
        ...
        assert synced_run.paper_revision_action_index is not None
        assert synced_run.paper_revision_action_index.revision_round == 1
        assert synced_run.paper_revision_action_index.pending_action_count == first_loop["pending_action_count"]
        assert synced_run.paper_revision_action_index.completed_action_count == first_loop["completed_action_count"]
        assert synced_run.paper_revision_diff is not None
        assert synced_run.paper_revision_diff.revision_round == 1
        assert synced_run.paper_revision_diff.base_revision_round == 0
        assert synced_run.paper_revision_diff.changed_section_count >= 1
        ...
        assert (
            "paper_sources/checkpoints/round_0001/paper_revision_action_index.json"
            in synced_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0001/revision_actions.md"
            in synced_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0001/paper_revision_diff.json"
            in synced_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0001/revision_diff.md"
            in synced_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
        ...
        assert Path(synced_run.paper_sources_dir or "", "checkpoints", "round_0001", "revision_actions.md").is_file()
        assert Path(synced_run.paper_sources_dir or "", "checkpoints", "round_0001", "paper_revision_action_index.json").is_file()
        assert Path(synced_run.paper_sources_dir or "", "checkpoints", "round_0001", "revision_diff.md").is_file()
        assert Path(synced_run.paper_sources_dir or "", "checkpoints", "round_0001", "paper_revision_diff.json").is_file()
...
""",
        ...
        assert resolved_run.paper_revision_action_index is not None
        assert resolved_run.paper_revision_action_index.revision_round == 2
        assert resolved_run.paper_revision_action_index.completed_action_count >= 1
        assert resolved_run.paper_revision_diff is not None
        assert resolved_run.paper_revision_diff.revision_round == 2
        assert resolved_run.paper_revision_diff.base_revision_round == 1
        assert resolved_run.paper_revision_diff.resolved_action_count >= 1
        ...
        assert (
            "paper_sources/checkpoints/round_0002/paper_revision_action_index.json"
            in resolved_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0002/revision_actions.md"
            in resolved_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0002/paper_revision_diff.json"
            in resolved_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        assert (
            "paper_sources/checkpoints/round_0002/revision_diff.md"
            in resolved_run.paper_revision_state.checkpoints[-1].relative_assets
        )
        ...
        assert Path(resolved_run.paper_sources_dir or "", "checkpoints", "round_0002", "revision_actions.md").is_file()
        assert Path(resolved_run.paper_sources_dir or "", "checkpoints", "round_0002", "paper_revision_action_index.json").is_file()
        assert Path(resolved_run.paper_sources_dir or "", "checkpoints", "round_0002", "revision_diff.md").is_file()
        assert Path(resolved_run.paper_sources_dir or "", "checkpoints", "round_0002", "paper_revision_diff.json").is_file()
        ...
        assert "# Revision Actions" in Path(
            resolved_run.paper_sources_dir or "", "revision_actions.md"
        ).read_text(encoding="utf-8")
        assert "# Revision Diff" in Path(
            resolved_run.paper_sources_dir or "", "revision_diff.md"
        ).read_text(encoding="utf-8")
    finally:
        client.close()
...
def test_autoresearch_paper_rebuild_preserves_revision_brief_from_review_loop(
    ...
    try:
        ...
        assert "paper_revision_diff.json" in {
            item["relative_path"] for item in rebuilt["paper_sources_manifest"]["files"]
        }
        assert "paper_revision_action_index.json" in {
            item["relative_path"] for item in rebuilt["paper_sources_manifest"]["files"]
        }
        assert "revision_actions.md" in {
            item["relative_path"] for item in rebuilt["paper_sources_manifest"]["files"]
        }
        assert "revision_diff.md" in {
            item["relative_path"] for item in rebuilt["paper_sources_manifest"]["files"]
        }
...
def test_autoresearch_save_run_preserves_revision_artifact_timestamps_when_unchanged(
    monkeypatch,
    tmp_path: Path,
) -> None:
    client = _configure_test_client(monkeypatch, tmp_path)
    try:
        project_id = _create_project(
            client,
            "Phase 5 Stable Revision Artifact Timestamps",
            "Automatic topic classification for compact CS abstracts",
        )
        run_response = client.post(
            f"/api/projects/{project_id}/auto-research/run",
        ...
        persisted_run = autoresearch_repository.load_run(project_id, run_id)
        assert persisted_run is not None
        assert persisted_run.paper_revision_action_index is not None
        assert persisted_run.paper_revision_diff is not None
        assert persisted_run.paper_section_rewrite_index is not None
        baseline_timestamps = (
            persisted_run.paper_revision_action_index.generated_at,
            persisted_run.paper_revision_diff.generated_at,
            persisted_run.paper_section_rewrite_index.generated_at,
        )

        saved_run = autoresearch_repository.save_run(persisted_run, touch_updated_at=False)
        assert saved_run.paper_revision_action_index is not None
        assert saved_run.paper_revision_diff is not None
        assert saved_run.paper_section_rewrite_index is not None
        assert (
            saved_run.paper_revision_action_index.generated_at,
            saved_run.paper_revision_diff.generated_at,
            saved_run.paper_section_rewrite_index.generated_at,
        ) == baseline_timestamps

        reloaded_run = autoresearch_repository.load_run(project_id, run_id)
        assert reloaded_run is not None
        assert reloaded_run.paper_revision_action_index is not None
        assert reloaded_run.paper_revision_diff is not None
        assert reloaded_run.paper_section_rewrite_index is not None
        assert (
            reloaded_run.paper_revision_action_index.generated_at,
            reloaded_run.paper_revision_diff.generated_at,
            reloaded_run.paper_section_rewrite_index.generated_at,
        ) == baseline_timestamps
    finally:
        client.close()
...
def test_autoresearch_publish_package_is_derived_from_selected_bundle(
    ...
    try:
        ...
        assert "run_paper_revision_action_index_json" in optional_roles
...
def test_autoresearch_operator_console_aggregates_current_run_state(
    ...
    try:
        ...
        assert console["runs"][0]["review_round"] == 1
        assert console["runs"][0]["open_issue_count"] >= 1
        assert console["runs"][0]["pending_action_count"] >= 1
        ...
        assert current["review_loop"]["current_round"] == 1
