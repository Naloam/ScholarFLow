...
def capability_note() -> str:
    ...
    return (
        ...
        "数据集都重新 load。先对每个数据集的全部 (claim, evidence)（≤500 条）一次性算好归一化句向量，"
        "再在 bootstrap 重采样里只做下标取值 + 余弦——Session 10 实测该路径在 512 seed × 500 例 × 3 数据集下"
        "计算量 <1 秒，10 分钟预算远未触顶，**seed 应贴近 512**（paired sign-flip 检验功效随 seed 数提升）。\n"
        "- baseline 分两层（Session 10）：① **弱基线** TF-IDF / Jaccard 词法相似度（仅证明句向量优于词法）；"
        "② **强基线 `stronger_baseline`** = BM25 (Okapi) 词法检索 + 浅层分数融合/阈值校准——proposed 的 win "
        "**必须越过强基线 BM25**，不能只赢弱基线 TF-IDF（VICTOR / VERIRAG 级基线见 Future Work，需 GPU/大模型，超 sandbox）。\n"
