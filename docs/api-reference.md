...
- connector statuses include cache hit/miss counts, network request counts, paper/error counts, `availability_status`, and `unavailable_reason`; deterministic cache misses are reported as unavailable source evidence rather than replaced with fake papers
