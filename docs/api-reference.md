...
- `final_publish_decision.json` records the Goal 7 final-publish policy decision with `policy_version`, `paper_tier`, passed/failed checks, blockers, required follow-ups, claim ceiling, evidence refs, archive manifest ref, and readiness manifest ref. `project_final_publish_ready` mirrors this decision, not loose package existence.
