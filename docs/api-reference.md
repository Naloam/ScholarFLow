...
- returns `domain_literature_strategy` and `domain_literature_result` when the brief has a routed domain; the result records source-class counts, fixture-only status, related-system coverage, known methods/datasets/metrics/SOTA, novelty risks, limitations, final-publish blockers, required follow-ups, kill criteria, and evidence refs
...
- RAG/citation-faithfulness and lightweight ML/NLP domain evidence currently uses deterministic review-only fixtures or import-replay-compatible protocols unless real benchmark/literature provenance is present; these domains can produce auditable review packages or concrete blockers, but their `domain_claim_ceiling` must remain scoped to review-only engineering validation until real multi-source evidence and publication-grade benchmark scale/provenance exist
