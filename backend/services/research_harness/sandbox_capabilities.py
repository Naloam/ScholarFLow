...
def domain_method_note(domain: str | None = None) -> str:
    ...
    if d == "tabular":
        return (
            ...
            "- 若假设是**后处理校准方法对比**（primary_metric=calibration_error，且系统是同一基分类器配不同 calibrator），"
            "则 systems = 校准器变体：baseline=未校准基分类器（直接用其 predict_proba），proposed=假设主推的校准器"
            "（temperature scaling / Platt scaling / isotonic regression 之一），ablation=其他校准器；**此时无需 RF/GB 强基线**，"
            "比较的是校准器之间——proposed 必须在 calibration_error 上胜过未校准 baseline。\n"
            ...
            "- 🔴 **方法论红线（每个 seed 必须遵守，违反即实验无效、metric 失真）**：\n"
            "  1. **真 held-out test**：每个 seed 用该 seed 切 train/test（如 `train_test_split(..., test_size=0.2, "
            "stratify=y, random_state=seed)`），**在 train 上 fit，在 test 上 predict + 算 metric**。"
            "**绝对禁止在训练数据上算 metric**（fit 后在 train 上 eval 会过拟合、metric 失真、制造 delta≈0 的假 null）。\n"
            "  2. **预处理只在 train 上 fit**：任何 scaler / 特征变换 / 校准器 **只在 train（或专用 calibration split）上 fit，"
            "再 transform/apply 到 test**。绝不能在 test 上 fit，也绝不能 fit 在 train 后漏掉对 test 的 transform"
            "（否则 train/test 分布不一致，所有缩放/校准系统会塌成相同的退化输出）。校准器尤其要 fit 在一个与训练/测试都"
            "不重叠的 calibration split（或用 calibration_curve 在 held-out 上拟合温度/Platt/isotonic）。\n"
            "  3. **proposed 与 baseline/ablation 代码路径必须实质不同**：不同的 fit / transform / predict 逻辑，"
            "不得复用同一函数导致跨 system 数值恒等（数值恒等 = 假消融/假对比）。\n"
            "  4. seed 数 ≥128；所有 system 共用同一组 (train_idx, test_idx) 以保证配对统计检验；"
            "模型/校准器在每个 seed 内重新 fit（不要跨 seed 复用 fit 结果）。\n"
        )
    if d == "structured":
        return (
